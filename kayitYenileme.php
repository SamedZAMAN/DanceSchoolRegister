                        ...
                        <caption>
                            ...
                            <?php 
                                ...
                                while($row = $result->fetch_assoc()){
                                    // KİŞİNİN BİLGİLER TABLOSUNDA OLUP OLMADIĞINI KONTROL ETME
                                    $tc = $row['tc'];
                                    $query = "SELECT * FROM bilgiler WHERE tc = ?";
                                    $statement = $mysqli->prepare($query);
                                    $statement->bind_param("i", $tc);
                                    $statement->execute();
                                    $result1 = $statement->get_result();
                                    
                                    if ($result1->num_rows > 0) {
                                        $adress = "addSecondMounth";
                                        $result1->free();
                                    } else {
                                        $adress = "addMounth";
                                        $result1->free();
                                    ...
                                    $i = ($row['id']==91) ? $i-=1 : $i = $i;
                                    echo "
                                        <tr class='$row[cinsiyet]' id='row_$row[id]'>
                                            ...
                                            <td class='$row[sınıf] sınıf' id='sinif_id$row[id]'><span id='sinif_$row[id]' >$row[sınıf]</span></td>
                                            <td id='edit_id$row[id]' class='edit_buttons'>
                                                <a class='btn btn-primary btn-sm edit-btn' id='edit-$row[id]' value='$row[id]' onClick='kayıt($row[id],$tname_json)'>EKle</a>
                                                <button class='btn btn-warning btn-sm edit-btn d-none' id='editbtn-$row[id]' name ='$adress' value='$row[id]' type='submit'>Kaydet</button>
                                            </td>
