<?php
  ...
  $tsql = "SELECT * FROM sınıflar";
    ...
    $psql = "SELECT * FROM para ORDER BY id DESC";
                    ...
                    <form action="update-process.php" method='post'>
                        ...
                        <caption>
                            ...
                            <tr>
                                ...
                                <th scope="col" class="hidden">Numara</th>
                                <th scope="col">Ödeme Tarihi</th>
                                <th scope="col" class="">Tutar</th>
                            ...
                            <?php 
                                ...
                                while($prow = $presult->fetch_assoc()){
                                    ...
                                    $bresult = $mysqli->query("SELECT * FROM bilgilercopy where id= $prow[user_id]");
                                    $row = $bresult->fetch_assoc();
                                    $row_classs = ($row['id']==91) ? 'hidden' : '';
                                    $miktarRengi = ($prow['miktar']>0) ? 'text-success' : 'text-danger';
                                    echo "
                                        <tr class='$row[cinsiyet]'>
                                            ...
                                            <td class='$row[number] hidden' id='number_id$row[id]'><span id='number_$row[id]'>$row[number]</span></td>
                                            <td class='$prow[tarih]' id='date_id$prow[id]'><span id='date_$prow[id]'>$prow[tarih]</span></td>
                                            <td class='$prow[miktar] sınıf $miktarRengi' id='sınıf_id$prow[id]'><span id='sınıf_$prow[id]' >$prow[miktar]</span></td>
                                            <td class='$prow[kime] sınıf' id='sınıf_id$prow[id]'><span id='sınıf_$prow[id]' >$prow[kime]</span></td>
                                        </tr>
                                    ";
                                $i +=1;
                                }
